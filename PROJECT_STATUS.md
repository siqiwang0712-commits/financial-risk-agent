...
## v0.3.4 — Hardened Boundaries & Verified Release Runtime

Status: release-candidate implementation. E1/E2/E3 and
`research/results/public_v1` are historical frozen artifacts and are not regenerated by
v0.3.4.

v0.3.4 is a hardening and architecture closeout release. The API, Agent and tool
registry reuse one configured `FinRiskPipeline`; runtime composition, transport schemas,
process isolation, frozen pilot projection and deployment-verification transport have
explicit owners. PDF inspection and analysis run behind killable process boundaries.
SEC and hosted-LLM endpoints, caches, document text and admitted evidence are validated
at their trust boundaries. The container release path builds once, verifies the candidate
digest through both the development overlay and operator-facing release Compose file,
then promotes that same digest.

The v0.3.3 foundation adds, on top of the v0.3.2 base: shared PostgreSQL rate limiting
with fail-closed behaviour, `DATABASE_URL_FILE` / `FINRISK_BOOTSTRAP_TOKEN_FILE`
mounted-secret support
(including in the migration job), bounded readiness probes that name a rejected
credential, a readiness probe that also verifies seven runtime schema sentinels
(`schema: "complete"`) rather than only that PostgreSQL accepts a connection, three
optional datastore timeouts (`FINRISK_DATABASE_OPERATION_TIMEOUT_SECONDS`,
`FINRISK_DB_RECONNECT_TIMEOUT_SECONDS`, `FINRISK_DB_CONNECT_TIMEOUT_SECONDS`) so an
outage answers in seconds instead of after the pool's 30-second default, and a Postgres
healthcheck that probes the container address instead of loopback.
...
v0.3.1 is the immutable historical empirical baseline. The current release gate covers
Python 3.11 and 3.12 with a 90% minimum coverage threshold. PostgreSQL 17
migration/restart persistence passed in both direct runtime validation and the
production-overlay Docker smoke, and remains mandatory in CI.
...
Status date: 2026-09-23. This remains a research prototype. Predictive superiority, population probability calibration, external validation, production SLA and regulatory compliance are not established.
...
- Deterministic scenario shocks, temporal trajectory classification, six-state disclosure-tension taxonomy, idempotent job queue semantics, hashed API-key abstraction, shared PostgreSQL rate limiting (with an in-process fallback for local runs) and tenant-safe document-storage protocol.
- Enterprise REST endpoints for the wired runtime: organizations, entities, risk cases and their lifecycle, portfolio overview, policies, analysis snapshots, temporal risk snapshots/timeline, applicability, selective decision, fusion, scenarios and audit events. The PostgreSQL migration additionally provisions documents, jobs, model registry, alerts and validation-record tables, plus the temporal evidence graph and shared rate-limit store; the reserved tables are schema-only and are not exposed as runtime endpoints.
...
- LLM integration: production adapter is implemented and mock-tested; v0.3.3 was smoke-tested against a real OpenAI-compatible endpoint (ChatAnywhere-hosted `gpt-5`, validated provider identity and schema-valid output, 17/17 provider + 11/11 PDF + 30/30 frozen-round assertions; mock fallback was not triggered). The validation credential is a temporary runtime key only and must be revoked/rotated before production use.
