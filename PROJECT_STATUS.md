...
Status: release-quality implementation. E1/E2/E3 and
`research/results/public_v1` are historical frozen artifacts and are not regenerated by
v0.3.3.

v0.3.3 adds, on top of the v0.3.2 base: shared PostgreSQL rate limiting with fail-closed
behaviour, `DATABASE_URL_FILE` / `FINRISK_BOOTSTRAP_TOKEN_FILE` mounted-secret support
...
v0.3.1 is the immutable historical empirical baseline. The v0.3.3 release gate covers
...
Status date: 2026-09-19. This remains a research prototype. Predictive superiority, population probability calibration, external validation, production SLA and regulatory compliance are not established.
...
- 27 frontend tests across four semantic test files, the CI-enforced official-registry production npm audit, a locally verified full npm audit, TypeScript checking and the Next.js standalone production build pass.
