...
Status: implementation is on `main`; final release-integrity changes remain local and
uncommitted. No v0.3.2 tag or release has been created. E1/E2/E3 and `research/results/public_v1` are historical frozen
...
The last published release is v0.3.1. The current v0.3.2 working tree collects and passes 143 Python tests on both Python 3.11.9 and 3.12.10 against a real PostgreSQL 17 service. Final coverage is 90.56%. E1/E2/E3 exact-byte hashes and the v0.3.0 `public_v1` snapshot are immutable historical evidence, not current-code regeneration targets.

Status date: 2026-09-10. This remains a research prototype. Predictive superiority, population probability calibration, external validation, production SLA and regulatory compliance are not established.
...
- 143 Python tests passed on Python 3.11.9 and 3.12.10, including the live PostgreSQL migration/restart persistence test.
- Local total line coverage is 90.56%; the enforced threshold is 90%.
- CI is configured for the same 143-test Python 3.11/3.12 matrix with PostgreSQL. Remote results are not claimed until these uncommitted closeout changes run in GitHub Actions.
- Four frontend semantic tests, TypeScript checking and the Next.js standalone production build pass.
- Ruff, E1/E2/E3 read-only hash replay and `git diff --check` pass.
- PostgreSQL 17 migration and restart persistence are locally runtime-validated on a dedicated test instance; production deployment remains unvalidated.
