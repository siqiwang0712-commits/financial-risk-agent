...
Status: implemented and locally validated. No v0.3.2 commit, tag or
release has been created. E1/E2/E3 and `research/results/public_v1` are historical frozen
...
The last published baseline is v0.3.1. The current v0.3.2 working tree passes 134 Python tests at 90.49% line coverage plus four frontend semantic tests, TypeScript and a production Next.js build. E1/E2/E3 exact-byte hashes and the v0.3.0 `public_v1` snapshot are immutable historical evidence, not current-code regeneration targets.
...
- 134 Python tests passed; one live-PostgreSQL test is skipped locally because no database server or Docker runtime is installed.
- Total line coverage: 90.49%; CI threshold remains 90%.
...
- PostgreSQL migration/restart persistence is configured as a CI service test but is not locally runtime-validated in this environment.
