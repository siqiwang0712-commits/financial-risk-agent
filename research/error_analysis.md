...
Every E3 FP/FN can be emitted as a structured case with one of: `EXTRACTION_ERROR`, `CONTEXT_ERROR`, `MODEL_APPLICABILITY`, `RULE_THRESHOLD`, `NARRATIVE_OVERWEIGHT`, `NUMERIC_OVERWEIGHT`, `MISSING_EVIDENCE`, `TEMPORAL_ERROR`, `FUSION_ERROR` or `LABEL_AMBIGUITY`. The six-row held-out endpoint produced one false negative for each executed numeric baseline and no false positives at the frozen threshold. These are machine-endpoint diagnostics, not human-adjudicated error rates.
