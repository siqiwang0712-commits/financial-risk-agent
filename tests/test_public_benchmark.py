...
def test_public_benchmark_is_company_disjoint_and_reproducible(tmp_path):
    ...
    assert (tmp_path / "summary.json").exists() and (
        tmp_path / "baseline-risk-f1.svg"
    ).exists()
    robustness = run_robustness_checks(manifest, ROOT)
    write_robustness(robustness, tmp_path)
...
def test_bootstrap_ci_is_seeded():
    assert bootstrap_ci([0, 1, 1], lambda x: sum(x) / len(x), 100) == bootstrap_ci(
        [0, 1, 1], lambda x: sum(x) / len(x), 100
    )
