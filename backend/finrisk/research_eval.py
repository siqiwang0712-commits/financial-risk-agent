...
def bootstrap_ci(
    ...
    estimates = sorted(
        statistic([rng.choice(items) for _ in items]) for _ in range(samples)
    )
...
def _ratio_probability(assessment) -> float:
    tests = [
        assessment.metrics["current_ratio"].value is not None
        and assessment.metrics["current_ratio"].value < 1,
        assessment.metrics["net_margin"].value is not None
        and assessment.metrics["net_margin"].value < 0,
        assessment.metrics["free_cash_flow"].value is not None
        and assessment.metrics["free_cash_flow"].value < 0,
        assessment.metrics["debt_to_assets"].value is not None
        and assessment.metrics["debt_to_assets"].value > 0.6,
    ]
...
def _model_probability(assessment) -> float:
    ...
    for model in assessment.models:
        ...
        if model.name == "Altman Z-Score":
            votes.append(float(model.output < 1.81))
        elif model.name == "Beneish M-Score":
            votes.append(float(model.output > -1.78))
        elif model.name == "Piotroski F-Score":
            votes.append(float(model.output <= 3))
        elif model.name == "Ohlson O-Score":
            votes.append(model.derived_outputs.get("probability", 0.0))
    return mean(votes) if votes else 0.5
...
def _probability(name: str, assessment) -> float | None:
    if name == "llm_only":
        ...
        return min(
            1.0,
            sum(
                "risk" in n["label"].lower() or "doubt" in n["label"].lower()
                for n in claims
            )
            / 2,
        )
    if name == "ratios_only":
        return _ratio_probability(assessment)
    if name == "rule_engine":
        return min(
            1.0,
            sum(
                s.score_delta
                for s in assessment.triggered_rules
                if not s.rule_id.startswith("MODEL_")
            )
            / 80,
        )
    if name == "traditional_models":
        return _model_probability(assessment)
...
def run_public_benchmark(
    ...
    if manifest.get("kind") != "public_company_reviewed":
        raise ValueError(
            "public benchmark must be explicitly marked public_company_reviewed"
        )
            ...
            rows.append(
                {
                    ...
                    "split": example["split"],
                    "baseline": baseline,
                    "risk_label": example["high_risk_label"],
                    "risk_probability": probability,
                    "risk_prediction": None
                    if probability is None
                    else int(probability >= 0.5),
    ...
    for baseline in BASELINES:
        ...
        pred = [r["risk_prediction"] for r in covered]
        cm = classification_metrics(true, pred)
        accuracy = mean([a == b for a, b in zip(true, pred)]) if covered else 0
        ci = bootstrap_ci(
            covered,
            lambda sample: mean(
                x["risk_label"] == x["risk_prediction"] for x in sample
            ),
        )
        ctrue = [r["contradiction_label"] for r in subset]
        cpred = [r["contradiction_prediction"] for r in subset]
        ...
        summaries.append(
            {
                ...
                "n_decisions": len(covered),
                "decision_coverage": round(len(covered) / len(subset), 4),
                "accuracy": round(accuracy, 4),
                "accuracy_ci_low": round(ci[0], 4),
                "accuracy_ci_high": round(ci[1], 4),
                "balanced_accuracy": round(balanced_accuracy(true, pred), 4),
                "risk_precision": round(cm.precision, 4),
                "risk_recall": round(cm.recall, 4),
                "risk_f1": round(cm.f1, 4),
                "auroc": None
                if roc_auc(true, probabilities) is None
                else round(roc_auc(true, probabilities), 4),
                "auprc": None
                if average_precision(true, probabilities) is None
                else round(average_precision(true, probabilities), 4),
                "brier": round(brier_score(true, probabilities), 4),
                "confusion_matrix": confusion_matrix(true, pred),
                "contradiction_f1": round(cmetrics.f1, 4),
                "evidence_coverage": round(
                    mean(r["evidence_coverage"] for r in subset), 4
                ),
                "ece": round(
                    expected_calibration_error(
                        probabilities, [bool(x) for x in true], bins=3
                    ),
                    4,
                ),
    ...
    for example in manifest["examples"]:
        ...
        variants = {
            ...
            "without_rules": (
                _probability("ratios_only", base)
                + _probability("traditional_models", base)
            )
            / 2,
            "without_models": (
                _probability("ratios_only", base) + _probability("rule_engine", base)
            )
            / 2,
        }
        no_trends = pipeline.assess(
            example["company"],
            example["fiscal_year"],
            example["current"],
            None,
            {int(k): v for k, v in example.get("pages", {}).items()},
            example["filing_url"],
        )
        ...
        decompositions.append(
            {
                ...
                "raw_current": example["current"],
                "raw_previous": example["previous"],
                "ratios": {k: v.value for k, v in assessment.metrics.items()},
                "models": [
                    {
                        "name": m.name,
                        "output": m.output,
                        "applicability": m.applicability,
                        "missing": m.missing_components,
                    }
                ...
                "narrative_pages": example.get("pages", {}),
                "contradictions": [
                    {
                        "category": c.category,
                        "claim": c.management_claim,
                        "conflicts": c.conflicting_evidence,
                    }
                ...
                "prediction": int(
                    ...
                    and assessment.overall_score >= 50
                ),
...
def run_robustness_checks(manifest_path: Path, root: Path) -> list[dict[str, Any]]:
    ...
    for example in manifest["examples"]:
        pages = {int(k): v for k, v in example.get("pages", {}).items()}
        for scale in (1, 1000):
            current = {k: v * scale for k, v in example["current"].items()}
            previous = {k: v * scale for k, v in example["previous"].items()}
            assessment = pipeline.assess(
                example["company"],
                example["fiscal_year"],
                current,
            ...
            rows.append(
                {
                    ...
                    else int(assessment.overall_score >= 50),
                    "confidence": assessment.confidence,
                }
            )
        keys = sorted(example["current"])
        for fraction in (0.1, 0.3):
            rng = random.Random(f"{example['id']}:{fraction}")
            drop = set(rng.sample(keys, max(1, int(len(keys) * fraction))))
...
def write_results(
    ...
    (output_dir / "score_decomposition.json").write_text(
        json.dumps(summary["decompositions"], indent=2), encoding="utf-8"
    )
    decomposition_rows = []
            ...
            decomposition_rows.append(
                {
                    ...
                    "drivers": "|".join(dimension["key_drivers"]),
                }
            )
    with (output_dir / "score_decomposition.csv").open(
        "w", newline="", encoding="utf-8"
    ) as handle:
        writer = csv.DictWriter(
            handle, fieldnames=list(decomposition_rows[0]), lineterminator="\n"
        )
        writer.writeheader()
    ...
    for item in summary["summaries"]:
        confusion_rows.append(
            {"baseline": item["baseline"], **item["confusion_matrix"]}
        )
    with (output_dir / "confusion_matrices.csv").open(
        "w", newline="", encoding="utf-8"
    ) as handle:
        writer = csv.DictWriter(
            handle, fieldnames=list(confusion_rows[0]), lineterminator="\n"
        )
        writer.writeheader()
    ...
    for i, item in enumerate(summaries):
        ...
        y = 285 - h
        bars.append(
            f'<rect x="{x}" y="{y:.1f}" width="90" height="{h:.1f}" rx="6" fill="#2f855a"/><text x="{x + 45}" y="{y - 8:.1f}" text-anchor="middle" font-size="15">{item["risk_f1"]:.2f}</text><text x="{x + 45}" y="315" text-anchor="middle" font-size="12">{item["baseline"]}</text>'
        )
